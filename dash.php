...
$sql = "SELECT * FROM vendeur ORDER BY id DESC";
$vendeurs = $conn->query($sql);
$nbr_vendeur = $vendeurs->rowCount();

//nbr prospect j1
$sql = "SELECT COUNT(id)  FROM vendeur WHERE date_ajout LIKE ? ";
$count_p = $conn->prepare($sql);
$count_p->execute(['%2023-03-11%']);
// $count_p = $conn->query($sql);
$prospec_j1 =  $count_p->fetch()['COUNT(id)'];


//nbr prospect j2
$sql = "SELECT COUNT(id)  FROM vendeur WHERE date_ajout LIKE ? ";
$count_p = $conn->prepare($sql);
$count_p->execute(['%2023-03-12%']);
// $count_p = $conn->query($sql);
$prospec_j2 =  $count_p->fetch()['COUNT(id)'];


//nbr prospect j3
$sql = "SELECT COUNT(id)  FROM vendeur WHERE date_ajout LIKE ? ";
$count_p = $conn->prepare($sql);
$count_p->execute(['%2023-03-23%']);
// $count_p = $conn->query($sql);
$prospec_j3 =  $count_p->fetch()['COUNT(id)'];
...
function get_agents_names($conn)
{
...
function count_vendeur_by_agent($conn)
{
  ...
  foreach ($stmt->fetchAll() as $user) {
    $sql = "SELECT COUNT(id) FROM vendeur WHERE agent = ?";
    $stmt2 = $conn->prepare($sql);
    $stmt2->execute([$user['id']]);
    $ligne = $stmt2->fetch();
    $dataset[] = $ligne['COUNT(id)'];
}
...
//Initialisation donnnée
$agents_names =  get_agents_names($conn);
$dataset =  count_vendeur_by_agent($conn);
        ...
        <div class="content">
                      ...
                      <div class="list-group-item">
                        <a class="notification notification-flush bg-200" href="#!">
                          ...
                          <div class="notification-body">
                            <p class="mb-1"><strong>Emma Watson</strong> replied to your comment : "Hello world ðÂÂÂÂÂÂ"</p>
                            <span class="notification-time"><span class="mr-1" role="img" aria-label="Emoji">ðÂÂÂÂ¬</span>Just now</span>
                      ...
                      <div class="list-group-item">
                        <a class="notification notification-flush" href="#!">
                          ...
                          <div class="notification-body">
                            ...
                            <span class="notification-time"><span class="mr-1" role="img" aria-label="Emoji">ðÂÂÂÂ¤ï¸ÂÂ</span>1d</span>
              ...
              <div class="card mb-3">
                <div class="card-body rounded-soft bg-gradient">
                  <div class="row text-white align-items-center no-gutters">
                    <div class="col">
                      <h4 class="text-white mb-0">Nbr. Prosects : <?=$nbr_vendeur?> </h4>
                      <p class="fs--1 font-weight-semi-bold">Nbr Agents <span class="opacity-50"><?=$nbr_agent?></span></p>
                    </div>
                    <div class="col-auto d-none d-sm-block">
                      <select class="custom-select custom-select-sm mb-3" id="dashboard-chart-select">
                        <!-- <option value="all">All Payments</option> -->
                        <option value="successful" selected="selected">Statisitique</option>
                        <!-- <option value="failed">Failed Payments</option> -->
                      </select>
                    </div>
                  </div>
                  <canvas class="max-w-100 rounded" id="my-chart-line" width="1618" height="375" aria-label="Line chart" role="img"></canvas>
                </div>
              </div>
            </div>
          ...
          <div class="card-deck">
            <div class="card mb-3 overflow-hidden" style="min-width: 12rem">
              ...
              <div class="card-body position-relative">
                <h6>Prospects Total<span class="badge badge-soft-warning rounded-capsule ml-2">
                  <span class="text-900 fs-1 fas fa-users"></span> 
                </span></h6>
                <div class="display-4 fs-4 mb-2 font-weight-normal text-sans-serif text-warning"><?= $nbr_vendeur ?></div>
              </div>
            </div>
            <div class="card mb-3 overflow-hidden" style="min-width: 12rem">
              ...
              <div class="card-body position-relative">
                <h6>Prospect J1 <span class="badge badge-soft-info rounded-capsule ml-2">0.0%</span></h6>
                <div class="display-4 fs-4 mb-2 font-weight-normal text-sans-serif text-info"><?=$prospec_j1?></div>
            ...
            <div class="card mb-3 overflow-hidden" style="min-width: 12rem">
              ...
              <div class="card-body position-relative">
                <h6>Prospesct J2 <span class="badge badge-soft-info rounded-capsule ml-2">0.0%</span></h6>
                <div class="display-4 fs-4 mb-2 font-weight-normal text-sans-serif text-info"><?=$prospec_j2?></div>
            ...
            <div class="card mb-3 overflow-hidden" style="min-width: 12rem">
              ...
              <div class="card-body position-relative">
                <h6>Prospesct PME <span class="badge badge-soft-info rounded-capsule ml-2">0.0%</span></h6>
                <div class="display-4 fs-4 mb-2 font-weight-normal text-sans-serif text-info"><?=$prospec_j3?></div>
          ...
          <div class="media mb-4 mt-2"><span class="fa-stack mr-2 ml-n1"><i class="fas fa-circle fa-stack-2x text-300"></i><i class="fa-inverse fa-stack-1x text-primary fas fa-list" data-fa-transform="shrink-2"></i></span>
            <div class="media-body">
              <h5 class="mb-0 text-primary position-relative"><span class="bg-200 pr-3">Les vendeurs enregistrés</span><span class="border position-absolute absolute-vertical-center w-100 z-index--1 l-0"></span></h5>
              <p class="mb-0">Listes des contact récoltés l'évenement.</p>
            </div>
          </div>

          <?php
          if ($vendeurs->rowCount() > 0) {
            ?>
          <div class="card mb-3">
            <div class="card-header">
                ...
                <table class="table table-sm mb-0 table-striped table-dashboard fs--1 data-table border-bottom border-200" data-options='{"searching":true,"responsive":false,"pageLength":32,"info":false,"lengthChange":false,"sWrapper":"falcon-data-table-wrapper","dom":"<&#39;row mx-1&#39;<&#39;col-sm-12 col-md-6&#39;l><&#39;col-sm-12 col-md-6&#39;f>><&#39;table-responsive&#39;tr><&#39;row no-gutters px-1 py-3 align-items-center justify-content-center&#39;<&#39;col-auto&#39;p>>","language":{"paginate":{"next":"<span class=\"fas fa-chevron-right\"></span>","previous":"<span class=\"fas fa-chevron-left\"></span>"}}}'>
                  <thead class="bg-200 text-900">
                    <tr>
                      ...
                      <th class="align-middle sort">Téléphone</th>
                      <th class="align-middle sort">Entreprise</th>
                      <th class="align-middle sort">Secteur Activicté</th>
                      <th class="align-middle sort">Agent</th>
                  ...
                  <tbody id="customers">
                    <?php 
                    while ($row = $vendeurs->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    <tr class="btn-reveal-trigger">
                      ...
                      <td class="py-2 align-middle white-space-nowrap"> <a ><?=$row['activite']?></a></td>
                      <td class="py-2 align-middle white-space-nowrap"> <a ><?=$row['secteur']?></a></td>
                      <td class="py-2 align-middle">
                        
                        <span class="badge badge-pill badge-soft-info fs-0" style="text-transform: uppercase;">
                          <?=getAgent($row['agent'],$conn)['name']?>
                      ...
                      <td class="align-middle text-center d-flex " style="cursor: pointer;" >
                          ...
                          <div class="wame" idPrd="{{ $prdL->id }}"  idVnt ="{{ $vente->id }}">
                            ...
                            <span class="mr-2  fas fa-2x fab fa-whatsapp text-success "  ></span>
                              
                            </a>
                          </div>
                            <div class="activeEdit" telVendeur="<?=$row['telephone']?>"  idVnt ="<?=$row['id']?>"
                                  VntPrdLine="<?=$row['id']?>"  >
                              <span class="mr-2 fas fa-2x far fa-copy text-warning "  ></span>
                            </div>
                            <div class="validEdit" telVendeur="<?=$row['telephone']?>"  idVnt ="<?=$row['id']?>"
                                  VntPrdLine="<?=$row['id']?>" >
...
$(function()
  ...
  utils.$document.ready(function () {
    ...
    var chartLine = document.getElementById('my-chart-line');
          ...
          tooltips: {
            ...
            callbacks: {
              label: function label(tooltipItem) {
                return labels[tooltipItem.index] + " - " + tooltipItem.yLabel + " Prospects";
              },
